...
void DFS_recursive(graph g,int start,int end){
	...
	for(int i=1;i<=g.num;i++){
		if(g.weight[start][i]==EDGE && g.check_visit[i]==NOVISIT){
			if(g.check_visit[end]==VISIT) break;
...
void DFS_iterative(graph g,int start,int end){
	...
	/* create stack */
	stack* pStack=create_stack(g.num);
	if(pStack==NULL){
		printf("ERROR: Stack is not created (in DFS)\n");
		return;
	}
	...
	printf("%s",arriveInEnd ? "" : "cannot find" );
