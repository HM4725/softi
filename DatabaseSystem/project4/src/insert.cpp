...
/* Inserts new values and its corresponding key into a leaf.
 */
void insert_into_leaf(int table_id, LeafPage* leaf_node, int64_t key, int64_t* values) {
    ...
    LEAF_KEY(leaf_node, insertion_point) = key;
    memcpy(LEAF_VALUE(leaf_node, insertion_point), values, SIZE_VALUE);
	leaf_node->num_keys++;
 ...
 */
void insert_into_leaf_after_splitting(int table_id, LeafPage* leaf, int64_t key,
        int64_t* values) {
    ...
    if (insertion_index < split) {
        ...
        LEAF_KEY(leaf, insertion_index) = key;
        memcpy(LEAF_VALUE(leaf, insertion_index), values, SIZE_VALUE);
        leaf->num_keys++;
    } else {
        ...
        memcpy(LEAF_VALUE(&new_leaf, insertion_index - split), values,
                SIZE_VALUE);
        new_leaf.num_keys++;
    }
 ...
 */
void insert_into_node(int table_id, InternalPage* n, int left_index, int64_t key,
        off_t right_offset) {
    int i;
...
void insert_into_new_root(int table_id, NodePage* left, int64_t key, NodePage* right) {
    ...
    header->root_offset = PAGENUM_TO_FILEOFF(root_node.pagenum);
...
void start_new_tree(int table_id, int64_t key, int64_t* values) {
    ...
    root_node.sibling = 0;
    memcpy(LEAF_VALUE(&root_node, 0), values, SIZE_VALUE);
    
    file_write_page(table_id, (Page*)&root_node);

    header->root_offset = PAGENUM_TO_FILEOFF(root_pagenum);
    file_write_page(table_id, (Page*)header);
}
...
int insert_record(int table_id, int64_t key, int64_t* values) {
	 ...
	 */
    if ((values_found = find_record(table_id, key)) != NULL) {
        free(values_found);
        return FAIL;
    }
	...
	if (leaf_node.num_keys < order_leaf - 1) {
        insert_into_leaf(table_id, &leaf_node, key, values);
	} else {
    	/* Case:  leaf must be split.
	     */
        insert_into_leaf_after_splitting(table_id, &leaf_node, key, values);
    }
    return 0;
}
