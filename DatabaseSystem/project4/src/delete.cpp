...
void remove_entry_from_node(int table_id, NodePage* node_page, int64_t key) {
	int i;
    int key_idx = 0;
...
void adjust_root(int table_id) {
    ...
    NodePage root_page;
    file_read_page(table_id, FILEOFF_TO_PAGENUM(header->root_offset), (Page*)&root_page);
	...
	if (!root_page.is_leaf) {
        ...
        header->root_offset = INTERNAL_OFFSET(root_node, 0);
		
        NodePage node_page;
        file_read_page(table_id, FILEOFF_TO_PAGENUM(header->root_offset),
                       (Page*)&node_page);
        node_page.parent = 0;
...
void redistribute_nodes(int table_id, NodePage* node_page, NodePage* neighbor_page,
	 ...
	 */
		...
		if (node_page->is_leaf) {
            ...
            NodePage child_page;
            file_read_page(table_id, FILEOFF_TO_PAGENUM(
                                    INTERNAL_OFFSET(node, node->num_keys + 1)),
                           (Page*)&child_page);
            child_page.parent = PAGENUM_TO_FILEOFF(node->pagenum);
			file_write_page(table_id, (Page*)&child_page);
...
void delete_entry(int table_id, NodePage* node_page, int64_t key) {
	...
	if (header->root_offset == PAGENUM_TO_FILEOFF(node_page->pagenum)) {
...
int delete_record(int table_id, int64_t key) {
    ...
    if ((value_found = find_record(table_id, key)) == NULL) {
